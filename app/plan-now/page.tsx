...
const benefits = [
  { icon: <PercentIcon />, label: "Descuento hasta los 30", desc: "Cuota reducida exclusiva para jóvenes de 18 a 30 años." },
  ...
  { icon: <ToothIcon />,   label: "Odontología sin límite", desc: "Preventiva y restauradora sin tope de edad." },
];

type PlanValue = string | boolean;
interface TableRow { id: string; category: string; label: string; plata: PlanValue; oro: PlanValue; }

// TODO: Reemplazar con datos reales del plan NOW
...
function CellValue({ value, hasDiff }: { value: PlanValue; hasDiff: boolean }) {
  ...
  return <span className={`text-sm ${hasDiff ? "text-[#a73a8d] font-semibold" : "text-gray-600"}`}>{value as string}</span>;
}

// ─── Page ─────────────────────────────────────────────────────────────────────

export default function PlanNowPage() {
          ...
          <section className="relative overflow-hidden bg-[#312664]" aria-label="Plan NOW — Plan joven William Hope">
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-28 lg:py-36">
              <div className="max-w-2xl">
                ...
                <RevealBlock>
                  <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-[#a73a8d]/50 bg-[#a73a8d]/20 text-[#a73a8d] text-xs font-bold uppercase tracking-wider mb-6">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#a73a8d]" aria-hidden="true" />
                    Plan joven · 18 a 30 años
                  </span>
                </RevealBlock>

                <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black text-white leading-[1.05] tracking-tight mb-6">
                  <SlideUp delay={60}>Descubrí</SlideUp>
                  <SlideUp delay={160}><span className="text-[#a73a8d]">NOW,</span></SlideUp>
                  <SlideUp delay={260}>el plan joven.</SlideUp>
                </h1>

                <RevealBlock delay={360}>
                  <p className="text-white/60 text-lg leading-relaxed mb-8 max-w-md">
                    Cobertura total pensada para tu ritmo de vida, con descuento especial hasta los 30 años.
                  </p>
                </RevealBlock>

                <RevealBlock delay={440}>
                  <div className="flex flex-wrap items-center gap-5">
                    <Link href="/quiero-afiliarme?plan=now" className="px-7 py-3.5 rounded-full gradient-brand text-white font-bold text-sm hover:opacity-90 transition shadow-lg shadow-[#a73a8d]/30">
                      Quiero el plan NOW
                    </Link>
                    <div className="flex items-center gap-2.5">
                      <span className="flex items-center justify-center w-8 h-8 rounded-full bg-[#a73a8d]/25 text-[#a73a8d]" aria-hidden="true">
                        <LightningIcon />
                      </span>
                      <p className="text-sm text-white/60">
                        <strong className="text-white font-semibold">Descuento especial</strong> 18–30
                      </p>
                    </div>
                  </div>
                </RevealBlock>
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-14">
                <RevealBlock className="mb-2">
                  <p className="text-[#a73a8d] text-xs font-bold uppercase tracking-widest">Beneficios</p>
                </RevealBlock>
                <h2 id="porque-title" className="text-3xl sm:text-4xl font-black text-[#312664]">
                  <SlideUp>¿Por qué NOW?</SlideUp>
                </h2>
              </div>
                ...
                {benefits.map((b, i) => (
                  <StaggerItem key={b.label} index={i}>
                    <div className="flex items-start gap-4 p-5 rounded-2xl bg-white border border-[#e4e7f0] h-full">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 text-[#a73a8d]" style={{ backgroundColor: "rgba(167,58,141,0.10)" }} aria-hidden="true">
                        {b.icon}
                      </div>
                      <div>
          ...
          <section className="py-24 bg-white" aria-labelledby="incluye-title">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="text-center mb-10">
                <RevealBlock className="mb-2">
                  <p className="text-[#a73a8d] text-xs font-bold uppercase tracking-widest">Coberturas</p>
                </RevealBlock>
                ...
                <RevealBlock delay={100}>
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold border transition-[background-color,color,border-color] duration-150 ${
                      showDiffsOnly
                        ? "bg-[#a73a8d] text-white border-[#a73a8d]"
                        : "bg-white text-[#312664] border-[#e4e7f0] hover:border-[#a73a8d]/50"
                    }`}
                    aria-pressed={showDiffsOnly}
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${showDiffsOnly ? "bg-white" : "bg-[#a73a8d]"}`} aria-hidden="true" />
                      ...
                      ...filteredRows.filter(r => r.category === cat).map(row => {
                        ...
                        return (
                          <tr key={row.id} className="border-b border-[#f6f6f9] hover:bg-[#fafafe] transition-colors">
                            ...
                            <td className="py-3.5 px-4 text-center tabular-nums border-x border-[#a73a8d]/8" style={{ backgroundColor: "rgba(167,58,141,0.025)" }}>
              ...
              <div className="md:hidden space-y-4">
                {(["plata", "oro"] as const).map(plan => {
                  ...
                  return (
                    <div key={plan} className={`rounded-2xl overflow-hidden border ${isOro ? "border-[#a73a8d]/25 shadow-lg shadow-[#a73a8d]/10" : "border-[#e4e7f0]"}`}>
                        ...
                        {filteredRows.map(row => {
                          ...
                          return (
                            <div key={row.id} className="flex items-center justify-between py-2.5">
                              ...
                              <span className={`text-sm flex-shrink-0 ${isOro && diff ? "text-[#a73a8d] font-semibold" : ""}`}>
          ...
          <section className="py-24 bg-[#f6f8fc]" aria-labelledby="clinicas-title">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="text-center mb-12">
                <RevealBlock className="mb-2">
                  <p className="text-[#a73a8d] text-xs font-bold uppercase tracking-widest">Red de prestadores</p>
                </RevealBlock>
                  ...
                  {clinics.map(c => (
                    <div key={c.name} className="w-full flex-shrink-0 bg-white">
                      ...
                      <div className="p-5">
                        ...
                        <p className="text-[#a73a8d] text-xs font-semibold uppercase tracking-wide mt-0.5 mb-3">{c.specialty}</p>
        ...
        <div className="sm:sticky sm:top-0 z-[42] rounded-t-[2.5rem] overflow-hidden shadow-[0_-16px_40px_rgba(0,0,0,0.10)]">
          <section className="gradient-brand py-20 px-4 sm:px-6 lg:px-8 text-center" aria-label="Afiliarse al plan NOW">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white leading-tight">
              <SlideUp delay={60}>Estamos cerca tuyo.</SlideUp>
              <SlideUp delay={180}>Donde vos estés.</SlideUp>
            </h2>
            <RevealBlock delay={320}>
              <Link href="/quiero-afiliarme?plan=now" className="inline-block mt-8 px-9 py-4 rounded-full bg-white text-[#312664] font-black text-sm hover:bg-white/90 transition shadow-xl shadow-black/15">
                ¡Quiero afiliarme!
              </Link>
            </RevealBlock>
          </section>
