...
function BuildingIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
      <path d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75" />
    </svg>
  );
}
function ChartIcon() {
...
const benefits = [
  { icon: <GroupIcon />,      label: "Para todo tu equipo",       desc: "Cobertura familiar extensible a cada empleado y su grupo." },
  { icon: <ShieldCheckIcon />, label: "Sin períodos de carencia",  desc: "Acceso inmediato a todas las prestaciones desde el día uno." },
  { icon: <BuildingIcon />,   label: "Gestión centralizada",      desc: "Un interlocutor dedicado para toda la empresa, siempre disponible." },
  { icon: <ChartIcon />,      label: "Flexibilidad de cobertura", desc: "Planes a medida según el tamaño y las necesidades de tu empresa." },
  { icon: <PercentIcon />,    label: "Descuentos corporativos",   desc: "Tarifas especiales según cantidad de empleados incorporados." },
  { icon: <HeadphonesIcon />, label: "Atención prioritaria",      desc: "Línea directa y prioridad en todos los servicios para empresas." },
];

type PlanValue = string | boolean;
interface TableRow { id: string; category: string; label: string; plata: PlanValue; oro: PlanValue; }

// TODO: Reemplazar con datos reales del plan EMPRESAS
...
export default function PlanEmpresasPage() {
          ...
          <section className="relative overflow-hidden bg-[#00A4B8]" aria-label="Plan EMPRESAS — William Hope">
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-28 lg:py-36">
              <div className="max-w-2xl">
                ...
                <RevealBlock>
                  <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-[#312664]/40 bg-[#312664]/15 text-[#312664] text-xs font-bold uppercase tracking-wider mb-6">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#312664]" aria-hidden="true" />
                    Plan empresas · Para tu equipo
                  </span>
                </RevealBlock>

                <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black text-white leading-[1.05] tracking-tight mb-6">
                  <SlideUp delay={60}>Descubrí</SlideUp>
                  <SlideUp delay={160}><span className="text-[#312664]">EMPRESAS,</span></SlideUp>
                  <SlideUp delay={260}>el plan corporativo.</SlideUp>
                </h1>

                <RevealBlock delay={360}>
                  <p className="text-white/70 text-lg leading-relaxed mb-8 max-w-md">
                    Cuidá la salud de tu equipo con cobertura total y beneficios para toda la empresa.
                  </p>
                </RevealBlock>
                ...
                <RevealBlock delay={440}>
                  <div className="flex flex-wrap items-center gap-5">
                    ...
                    <div className="flex items-center gap-2.5">
                      <span className="flex items-center justify-center w-8 h-8 rounded-full bg-[#312664]/20 text-[#312664]" aria-hidden="true">
                        <BriefcaseIcon />
                      </span>
                      <p className="text-sm text-white/70">
                        <strong className="text-white font-semibold">Desde 2 empleados</strong>
                      </p>
                    </div>
                  </div>
                </RevealBlock>
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-14">
                <RevealBlock className="mb-2">
                  <p className="text-[#00A4B8] text-xs font-bold uppercase tracking-widest">Beneficios</p>
                </RevealBlock>
                <h2 id="porque-title" className="text-3xl sm:text-4xl font-black text-[#312664]">
                  <SlideUp>¿Por qué EMPRESAS?</SlideUp>
                </h2>
              </div>
                ...
                {benefits.map((b, i) => (
                  <StaggerItem key={b.label} index={i}>
                    <div className="flex items-start gap-4 p-5 rounded-2xl bg-white border border-[#e4e7f0] h-full">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 text-[#00A4B8]" style={{ backgroundColor: "rgba(0,164,184,0.10)" }} aria-hidden="true">
                        {b.icon}
                      </div>
                      <div>
          ...
          <section className="py-24 bg-white" aria-labelledby="incluye-title">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="text-center mb-10">
                <RevealBlock className="mb-2">
                  <p className="text-[#00A4B8] text-xs font-bold uppercase tracking-widest">Coberturas</p>
                </RevealBlock>
                ...
                <RevealBlock delay={100}>
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold border transition-[background-color,color,border-color] duration-150 ${
                      showDiffsOnly
                        ? "bg-[#00A4B8] text-white border-[#00A4B8]"
                        : "bg-white text-[#312664] border-[#e4e7f0] hover:border-[#00A4B8]/50"
                    }`}
                    aria-pressed={showDiffsOnly}
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${showDiffsOnly ? "bg-white" : "bg-[#00A4B8]"}`} aria-hidden="true" />
                ...
                <table className="w-full border-collapse" aria-label="Comparativa PLATA vs ORO">
                    ...
                    {visibleCategories.flatMap(cat => [
                      <tr key={`cat-${cat}`} className="border-t-2 border-[#f0f0f5]">
                        <td colSpan={3} className="pt-5 pb-2">
                          <span className="text-xs font-bold text-[#00A4B8] uppercase tracking-wider">{cat}</span>
                        </td>
                      </tr>,
                      ...filteredRows.filter(r => r.category === cat).map(row => {
                        const diff = String(row.plata) !== String(row.oro);
                        return (
                          <tr key={row.id} className="border-b border-[#f6f6f9] hover:bg-[#f6feff] transition-colors">
                            ...
                            <td className="py-3.5 px-4 text-center tabular-nums border-x border-[#00A4B8]/10" style={{ backgroundColor: "rgba(0,164,184,0.03)" }}>
                  ...
                  <tfoot>
                    <tr>
                      ...
                      <td className="pt-8 pb-2 px-4">
                        <Link href="/quiero-afiliarme?plan=oro" className="block py-3 px-6 rounded-full gradient-brand text-white text-center font-bold text-sm hover:opacity-90 transition shadow-md shadow-[#00A4B8]/15">
                          Elegir ORO
                        </Link>
                      </td>
                    </tr>
              ...
              <div className="md:hidden space-y-4">
                {(["plata", "oro"] as const).map(plan => {
                  ...
                  return (
                    <div key={plan} className={`rounded-2xl overflow-hidden border ${isOro ? "border-[#00A4B8]/25 shadow-lg shadow-[#00A4B8]/10" : "border-[#e4e7f0]"}`}>
          ...
          <section className="py-24 bg-[#f6f8fc]" aria-labelledby="clinicas-title">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="text-center mb-12">
                <RevealBlock className="mb-2">
                  <p className="text-[#00A4B8] text-xs font-bold uppercase tracking-widest">Red de prestadores</p>
                </RevealBlock>
                  ...
                  {clinics.map(c => (
                    <div key={c.name} className="w-full flex-shrink-0 bg-white">
                      ...
                      <div className="p-5">
                        ...
                        <p className="text-[#00A4B8] text-xs font-semibold uppercase tracking-wide mt-0.5 mb-3">{c.specialty}</p>
              ...
              <div className="flex items-center justify-center gap-2.5 mt-8">
                ...
                {clinics.map((c, i) => (
                  <button
                    ...
                    className={`flex items-center py-3 rounded-full transition-[width] duration-200 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#00A4B8]/50 group ${i === currentClinic ? "w-6" : "w-2"}`}
                    aria-label={c.name}
                    aria-current={i === currentClinic ? "true" : undefined}
                  >
                    <span className={`block h-2 w-full rounded-full transition-colors duration-200 ${i === currentClinic ? "bg-[#00A4B8]" : "bg-[#00A4B8]/30 group-hover:bg-[#00A4B8]/60"}`} />
                  </button>
                ))}
        ...
        <div className="sm:sticky sm:top-0 z-[42] rounded-t-[2.5rem] overflow-hidden shadow-[0_-16px_40px_rgba(0,0,0,0.10)]">
          <section className="gradient-brand py-20 px-4 sm:px-6 lg:px-8 text-center" aria-label="Afiliarse al plan EMPRESAS">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white leading-tight">
              <SlideUp delay={60}>Invertí en</SlideUp>
